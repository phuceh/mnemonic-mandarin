...
response = supabase.table("vocabulary").select("word_id, chinese").eq("hsk_level", 3).execute()
words = response.data
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix='.mp3', delete=False) as tmp:
            ...
            with open(tmp.name, 'rb') as f:
                supabase.storage.from_("audio").upload(filename, f, {"content-type": "audio/mpeg", "upsert": "true"})
        os.unlink(tmp.name)
